...
const Page: React.FC = () => {
  ...
  const handleAddQuantity = (product: ProductTypes) => {
    ...
    if (Number(product.available_stocks) < qty) return // return if it exceeds available stocks
      ...
      <div className='md:flex md:space-x-4 space-y-2 md:space-y-0 mx-2'>
                            ...
                            cart.map((product: ProductTypes, index: number) =>
                              <tr key={index} className="app__tr">
                                <td className="app__td">
                                  <XMarkIcon onClick={() => handleRemoveFromCart(product)} className='w-5 h-5 text-red-600 bg-red-200 border border-red-300 cursor-pointer'/>
                                </td>
                                ...
                                <td className="hidden md:table-cell app__td">
                                  <span className='font-bold text-sm'>{product.total}</span>
                                </td>
                              </tr>
                            )
                          }
        ...
        <div className='w-full md:w-1/3'>
          <div className='w-full bg-green-600 px-4 py-2'>
            <div className='text-white'>TOTAL: <span className='font-bold'>{cartTotal}</span></div>
          </div>
          {
            (cartTotal < 1)
              ? <div className='p-4 bg-gray-100 border border-gray-200 shadow-md'>
