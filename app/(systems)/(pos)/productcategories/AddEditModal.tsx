...
// Types
import type { AccountTypes, ProductCategoryTypes } from '@/types'
...
interface ModalProps {
  hideModal: () => void
  editData: ProductCategoryTypes | null
}
...
const AddEditModal = ({ hideModal, editData }: ModalProps) => {
  ...
  const { register, formState: { errors }, reset, handleSubmit } = useForm<ProductCategoryTypes>({
    mode: 'onSubmit'
  })

  const onSubmit = async (formdata: ProductCategoryTypes) => {
    if (saving) return
  ...
  const handleCreate = async (formdata: ProductCategoryTypes) => {
    try {
      const newData = {
        name: formdata.name,
  ...
  const handleUpdate = async (formdata: ProductCategoryTypes) => {
    if (!editData) return
                ...
                ? <>
                    <div className='app__form_field_container'>
                      <div className='w-full'>
                        <div className='app__label_standard'>Category Name</div>
                        <div>
                          <input
                            {...register('name', { required: true })}
                            type='text'
                            className='app__select_standard'/>
                          {errors.name && <div className='app__error_message'>Category Name is required</div>}
                        </div>
                      </div>
                    </div>
                  </>
