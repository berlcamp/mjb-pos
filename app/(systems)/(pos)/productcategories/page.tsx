...
import { fetchProductCategories } from '@/utils/fetchApi'
...
// Types
import type { ProductCategoryTypes } from '@/types'
...
const Page: React.FC = () => {
  ...
  const [list, setList] = useState<ProductCategoryTypes[]>([])
  ...
  const [editData, setEditData] = useState<ProductCategoryTypes | null>(null)
  ...
  const fetchData = async () => {
    ...
    try {
      const result = await fetchProductCategories({ filterKeyword, filterStatus }, perPageCount, 0)
  ...
  const handleShowMore = async () => {
    ...
    try {
      const result = await fetchProductCategories({ filterKeyword, filterStatus }, perPageCount, list.length)
  ...
  const handleEdit = (item: ProductCategoryTypes) => {
    setShowAddModal(true)
    setEditData(item)
  }
      ...
      <div>
          ...
          <div>
            <table className="app__table">
              ...
              <tbody>
                {
                  !isDataEmpty && list.map((item: ProductCategoryTypes) => (
                    <tr
                      key={uuid()}
                      className="app__tr">
                      <td
