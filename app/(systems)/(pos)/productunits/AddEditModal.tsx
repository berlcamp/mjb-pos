...
// Types
import type { AccountTypes, ProductUnitTypes } from '@/types'
...
interface ModalProps {
  hideModal: () => void
  editData: ProductUnitTypes | null
}
...
const AddEditModal = ({ hideModal, editData }: ModalProps) => {
  ...
  const { register, formState: { errors }, reset, handleSubmit } = useForm<ProductUnitTypes>({
    mode: 'onSubmit'
  })

  const onSubmit = async (formdata: ProductUnitTypes) => {
    if (saving) return
  ...
  const handleCreate = async (formdata: ProductUnitTypes) => {
    try {
      const newData = {
        name: formdata.name,
  ...
  const handleUpdate = async (formdata: ProductUnitTypes) => {
    if (!editData) return
