...
import { Sidebar, PerPage, TopBar, TableRowLoading, ShowMore, Title, Unauthorized, CustomButton, ProjectsSideBar, UserBlock, ConfirmModal } from '@/components'
...
const Page: React.FC = () => {
  ...
  const [list, setList] = useState<ProjectTypes[]>([])
  ...
  const [editData, setEditData] = useState<ProjectTypes | null>(null)
  ...
  const fetchData = async () => {
    ...
    try {
      const result = await fetchProjects({ filterKeyword, filterStatus }, perPageCount, 0)
  ...
  const handleShowMore = async () => {
    ...
    try {
      const result = await fetchProjects({ filterKeyword, filterStatus }, perPageCount, list.length)
  ...
  const handleEdit = (item: ProjectTypes) => {
    setShowAddModal(true)
    setEditData(item)
  }
      ...
      <div>
          ...
          <div>
            <table className="app__table">
              ...
              <tbody>
                {
                  !isDataEmpty && list.map((item: ProjectTypes) => (
                    <tr
                      key={uuid()}
                      className="app__tr">
                      <td
