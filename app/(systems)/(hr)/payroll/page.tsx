...
const Page: React.FC = () => {
  ...
  const [selectedPayroll, setSelectedPayroll] = useState<PayrollTypes | null>(
    null
  )
  ...
  const handleGenerateSummary = async (item: PayrollTypes) => {
    ...
    const titleText3 = `${format(
      new Date(item.from),
      'MMM dd, yyyy'
    )} - ${format(new Date(item.to), 'MMM dd, yyyy')}`
    const titleWidth =
    ...
    payrollEmployees.sort(
      (a: PayrollEmployeeTypes, b: PayrollEmployeeTypes) => {
    ...
    const grossTotal = payrollEmployees.reduce(
      (accumulator: number, item: PayrollEmployeeTypes) =>
        accumulator + Number(item.gross_pay),
      0
    )
    const netTotal = payrollEmployees.reduce(
      (accumulator: number, item: PayrollEmployeeTypes) =>
        accumulator + Number(item.net_pay),
      0
    )
    const caDeductionTotal = payrollEmployees.reduce(
      (accumulator: number, item: PayrollEmployeeTypes) =>
        accumulator + Number(item.ca_deduction),
      0
    )

    data.push({
      days: 'Total: ',
      gross_pay: grossTotal.toLocaleString('en-US'),
      ca_deduction: caDeductionTotal.toLocaleString('en-US'),
      net_pay: netTotal.toLocaleString('en-US'),
    })
  ...
  if (
    !(hasAccess('human_resource') || hasAccess('cashers')) &&
            ...
            <table className="app__table">
              <thead className="app__thead">
                <tr>
                  ...
                  <th className="hidden md:table-cell app__th">Period</th>
                ...
                {!isDataEmpty &&
                  list.map((item: PayrollTypes) => (
                    <tr
                            ...
                            <Menu.Items className="app__dropdown_items">
                              <div className="py-1">
                                ...
                                <Menu.Item>
                                  <div
                                    onClick={async () =>
                                      await handleGenerateSummary(item)
                                    }
                                    className="app__dropdown_item">
                      ...
                      <th className="app__th_firstcol">
                        ...
                        <div>
                          <div className="md:hidden app__td_mobile">
                            <div>
                              Date:{' '}
                              {format(new Date(item.from), 'MMMM dd, yyyy')} -{' '}
                              {format(new Date(item.to), 'MMMM dd, yyyy')}
                            </div>
                      ...
                      <td className="hidden md:table-cell app__td">
                        {format(new Date(item.from), 'MMM dd, yyyy')} -{' '}
                        {format(new Date(item.to), 'MMM dd, yyyy')}
                      </td>
